...
def build_reports():
    ...
    reports = [
        # (giữ nguyên các report có sẵn của bạn)
        ...
        # 1) Chỉ EXT_URL (mọi loại nội dung)
        {"name": "Traffic sources (EXT_URL only)",
         "dimensions": "insightTrafficSourceType",
         "metrics": base_mets + avg_mets + ["engagedViews"],
         "params": {"filters": "insightTrafficSourceType==EXT_URL"}},  # EXT_URL là referrer web ngoài

        # 2) Tất cả traffic source nhưng CHỈ Shorts
        {"name": "Traffic sources (SHORTS only)",
         "dimensions": "insightTrafficSourceType",
         "metrics": base_mets + avg_mets + ["engagedViews"],
         "params": {"filters": "creatorContentType==SHORTS"}},  # Lọc theo loại nội dung Shorts

        # 3) Chi tiết trang giới thiệu CHỈ EXT_URL cho Shorts
        {"name": "Traffic source detail (SHORTS + EXT_URL)",
         "dimensions": "insightTrafficSourceDetail",
         "metrics": base_mets + avg_mets + ["engagedViews"],
         "params": {
             "filters": "creatorContentType==SHORTS;insightTrafficSourceType==EXT_URL",
             "sort": "-views"
         }},
...
# =========================
def run_reports_to_csv(credentials, account_tag: str, channel_id: str):
    yta = build("youtubeAnalytics", "v2", credentials=credentials)
    start_date, end_date = get_date_range()
    reports = build_reports()

    # Thư mục con: reports/<account_tag>/<channel_id>/
    account_dir = os.path.join(REPORT_ROOT, account_tag, channel_id)
    ...
    with open(meta_path, "w", encoding="utf-8") as mf:
        ...
        mf.write(f"Channel ID: {channel_id}\n")
    ...
    for spec in reports:
        ...
        req = {
            "ids": ("contentOwner==" + CONTENT_OWNER_ID) if IS_OWNER_MODE else f"channel=={channel_id}",
...
def main():
    ...
    def process_one(cred_file: str):
        ...
        channels = get_youtube_data(creds)
        print(f"  Found {len(channels)} channel(s) under this credential.")
        if not channels:
        ...
        for ch in channels:
            ...
            run_reports_to_csv(creds, account_tag, ch["channel_id"])
